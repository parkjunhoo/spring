...
<script type="text/javascript">
			...
			$("#guguajaxbtn").on("click", function() {
				...
				$.ajax({
					...
					success:function(data){
						//ajax로 요청하는 경우 response되는 데이터는 무조건 success에서 실행되는 함수
						//내부에서만 사용할 수 있다.
						$("#result2").html("<h2>jquery:"+data+"</h2>");
					},
					error:function(obj,msg,statusMsg){
						//ajax요청이 실패하는 경우
						//화면을 비워둘 수 없으므로 요청이 실패한 경우 만들어질 페이지를 출력하기 위한 목적으로
						//사용
						alert("실패");	
					}
		...
		//ajax요청이 성공하면 success_run이 자동으로 호출되면 서버의 실행 결과가 success_run의 
		//매개변수로 전달이 된다.
